...
from refseg.models.models import RefSegEncoderDecoder, RefSegSam, RefSegSiglipTextModel, RefSegSiglipVisionModel, CascadedPrompter, PseudoSegHead

default_scope = 'mmseg'
custom_imports = dict(imports=['refseg'], allow_failed_imports=False)

work_dir = f'./work_dirs/refsegrs'

data_root = f'/mnt/ali-sh-1/dataset/cky_data/RefSegRS/images'
...
vis_backends = [
    ...
    dict(type=WandbVisBackend, init_kwargs=dict(project='RefSeg', group='refsegrs-l', name=work_dir.split('/')[-1]))
]
...
train_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='datainfo/refsegrs_train.jsonl',
...
val_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='datainfo/refsegrs_test.jsonl',
